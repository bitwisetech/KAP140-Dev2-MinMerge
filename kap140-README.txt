...
This implementation is built according to the user manual.
You can set the desired autopilot variant in the kap140-config.xml (see comments there).
...
Configure navigation data sources in nav-selector-config.xml, examples are there.
This enables you to use up to three navigation sources (nav radios, rnav devices, GPS, etc...).
Property '/instrumentation/nav-source/selector' selects the data source used by autopilot. It can be set to 0, 1 or 2.
If you have ony one source, configure only the first and avoid a switch or button in the cockpit.

The nav-selector is only a bunch of filters to pull all information to one location.

The AP navigates according to the information under '/instrumentation/nav-source', 
it does not care where the information came from.

If the KAP140 isn't tied with an altimeter, the standard pressure will flash after Pre Flight Test (PFT) is finished.
...
If not, there is no radial pointer, therefore the KAP140 uses the heading bug for the radial.
...
After 5 seconds (blinking HDG stops), the KAP140 takes the heading bug as radial.
If the nav radio is tuned to an ILS, always set the heading bug to the inbound course, even if you use REV!
...
This means, the deviation needle is only 3 points left or right off the center (or closer).
The actual scaling of the points doesn't matter. Normally, on VOR one point represents 2 degrees and
on LOCalizer (ILS) one point represents around 0.8 degrees. Often on RNAV one point represents 1 NM.
But the KAP140 doesn't know the scaling. It catches inside 3 points left or right.

If the KAP140 is in NAV, APR or REV mode and the deviation needle moves more than 3 points away,
the mode starts flashing. The autopilot will try to come back on the right track.
If the autopilot isn't back in the next 30 seconds, it automatically disengages.

If the autopilot is in VS mode, one press on UP or DOWN button changes the display to show the actual FPM without altering it.
Subsequent presses on the UP or DOWN button, while the FPM is shown in the display, will alter the rate by 100fpm.
...
- rename one of the texture files to 'kap140.png' (there is a dark and bright texture to choose from)
...
- add the 'kap140.xml' to your panel (like every other model)
    ...
    make sure that these two objects have a transparent effect!

- if your aircraft doesn't have an encoding altimeter, add a 'altimeter' in your instrumentation
...
- include the file 'kap140-properties.xml' in your set-file
...
make sure that these files can find 'kap140-config.xml' (change the path in 'include' clause if necessary).

- include the file 'nav-selector-properties.xml' in your set-file
    an example if you dont have a 'instrumentation' section:
...
- include the file 'kap140.nas' under '<nasal>' in your set-file
    as example:
        ...
        <nasal>
            …
            <kap140>
              <file>Nasal/kap140.nas</file>
            </kap140>
            …
        </nasal>

Important: the file 'kap140.nas' has on the very first line a command to read in
the file 'kap140-config.xml'!
make sure that the script can read 'kap140-config.xml' (change the path if necessary).

- move the sound files (kap140-alert.wav and kap140-disengage.wav) to your sound directory
...
The generic AP dialog will not work with the KAP 140, so its a good idea to include this dialog.

- move the file 'kap140-dlg.xml' to '/gui/dialogs/'
    If you don't have these directories, create them.
    Dialog must be in this directory, otherwise FG can will not find it.


KAP 140 key bindings:
---------------------

The KAP140 supports disconnect by keypress (AP disconnect).
...
The KAP140 supports CWS (Control Wheel Steering).
To use this feature, add the following lines to your keyboard.xml:
...
For this purpose the KAP 140 defines properties under "/autopilot/kap140/event/".

Every button is represented by a boolean property.
- if you press the button, the property is set to 'true'
- if you release the button, the property is set to 'false'
...
Every knob is represented by an integer property.
- if you rotate the knob forward, the property is incremented
- if you rotate the knob backward, the property is decremented
...
This way, you can control the KAP 140 via a joystick, telnet or even via serial link (with RaspberryPI or Arduino).
